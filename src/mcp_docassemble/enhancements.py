...
class DocassembleClientEnhanced:
    ...
    def _detect_docassemble_version(self) -> Optional[str]:
        ...
        try:
            ...
            if config and isinstance(config, dict):
                ...
                version = config.get('version') or config.get('build') or config.get('da_version')
                if version:
        ...
        try:
            ...
            if 'Docassemble' in response.text:
                ...
                import re
                version_match = re.search(r'Docassemble[^0-9]*([0-9]+\.[0-9]+[^"<\s]*)', response.text)
                if version_match:
        ...
        logger.warning("Could not detect Docassemble version, using default compatibility")
        return "unknown"
    ...
    def _graceful_fallback(self, feature_name: str, fallback_result: Any = None, 
        ...
        if not self.enable_fallbacks:
            raise DocassembleAPIError(
                error_message or f"Feature '{feature_name}' not supported in this Docassemble version"
            )
    ...
    def enhanced_start_interview(self, i: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.enable_fallbacks:
                return self._graceful_fallback('start_interview', 
    ...
    def enhanced_get_interview_variables(self, session: str, i: str, **kwargs):
        ...
        try:
            ...
            if "Unable to obtain interview" in str(e):
                if self.enable_fallbacks:
                    logger.warning(f"Session {session} expired or invalid, providing fallback")
                    return {
                        "variables": {},
