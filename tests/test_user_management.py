...
class UserManagementTests(APITestBase):
    ...
    def run_all_tests(self) -> Dict[str, Tuple[bool, str]]:
        ...
        results['list_users'] = self.test_endpoint(
            'list_users', self._test_list_users
        )
    ...
    def _test_give_user_privilege(self):
        ...
        test_user = next((u for u in users['items'] if 'admin' not in u['privileges'] and u['id'] != 1), None)
        if test_user:
            return self.client.give_user_privilege(user_id=test_user['id'], privilege='advocate')
        else:
    ...
    def _test_remove_user_privilege(self):
        ...
        users = self.client.list_users()
        test_user = next((u for u in users['items'] if len(u['privileges']) > 0 and u['id'] != 1), None)
        if test_user and test_user['privileges']:
            return self.client.remove_user_privilege(
                user_id=test_user['id'], 
                privilege=test_user['privileges'][0]
            )
        else:
