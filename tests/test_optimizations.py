...
class SessionHandlingTests(BaseTest):
    ...
    def _test_go_back_with_session(self):
        ...
        try:
            ...
            result = self.client.go_back_in_interview(session=session_id, steps=1)
...
class OptimizedTestRunner:
    ...
    def __init__(self):
        self.tests = [OptimizedFailureTests(), SessionHandlingTests()]
            ...
            for test_name, (result, error) in suite_results["results"].items():
                if result is not None:
                    ...
                    results["improvements"][
                        test_name
                    ] = f"❌ Weiterhin problematisch: {error}"
